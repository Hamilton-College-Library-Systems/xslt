...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:dhi="http://www.dhinitiative.org/dhi/" exclude-result-prefixes="dhi" version="2.0">
      ...
      <xsl:for-each select="child::node()">
         <xsl:if test="child::node() != ''">
            <row>
               ...
               <xsl:call-template name="requiredFields"/>
   ...
   <xsl:template name="requiredFields">
      <xsl:for-each select="//root/row">
         <xsl:choose>
            <xsl:when test="not(typeOfResource)">**** Oops: the "typeOfResource" field is missing. *** <xsl:message terminate="yes">Processing terminated.</xsl:message></xsl:when>
            <xsl:when test="not(genre_u-primary_)">**** Oops: the "genre_u-primary_" field is missing. *** <xsl:message terminate="yes">Processing terminated.</xsl:message></xsl:when>
            <xsl:when test="not(genre_t-subgenre_)">**** Oops: the "genre_t-subgenre_" field is missing. *** <xsl:message terminate="yes">Processing terminated.</xsl:message></xsl:when>
            <xsl:when test="not(language)">**** Oops: the "language" is missing. *** <xsl:message terminate="yes">Processing terminated.</xsl:message></xsl:when>
            <xsl:otherwise/>
         </xsl:choose>
      </xsl:for-each>
   </xsl:template>
